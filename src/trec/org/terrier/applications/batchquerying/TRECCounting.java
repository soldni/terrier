 ...
 * under the License.
 *
 * The Original Code is TRECCounting.java.
 *
 ...
 * 	 Luca Soldaini <luca{a.}ir.cs.georgetown.edu>
 */
 ...
 * <h2>Configuring</h2>
 * <p>In the following, we list the main ways for configuring TRECCounting,
 * before exhaustively listing the properties that can affect TRECCounting.
 *
 * <h3>Topics</h3>
 ...
 * commas. By default TRECCounting assumes TREC tagged topic files, e.g.:
 * <pre>
 * &lt;top&gt;
 * &lt;num&gt; Number 1 &lt;/num&gt;
...
public class TRECCounting {
	...
	/** The logger used */
	protected static final Logger logger = Logger.getLogger(TRECCounting.class);
	...
	/**
	 * TRECCounting default constructor initialises the inverted index, the
	...
	/**
	 * TRECCounting constructor initialises the inverted index, the
	 * lexicon and the document index structures.
	 */
	public TRECCounting(boolean _queryexpansion) {
		this.loadIndex();
		this.createManager();
		this.querySource = this.getQueryParser();
		this.printer = getOutputFormat();
	...
	/**
	 * TRECCounting constructor initialises the specified inverted index, the
	 * lexicon and the document index structures.
	 *
	 * @param i The specified index.
	 */
	public TRECCounting(Index i) {
		this.setIndex(i);
		this.createManager();
		this.querySource = this.getQueryParser();
		this.printer = getOutputFormat();
	...
	protected QueryResultCache getResultsCache() {
		...
		try {
			...
			if (!className.contains("."))
				className = "org.terrier.applications.TRECCounting$"
						+ className;
	...
	public void printSettings(final SearchRequest default_q,
		...
		try {
			...
			ApplicationSetup.getUsedProperties().store(
					bos,
					" Settings of Terrier (TRECCounting) generated for run "
							+ resultsFilename);
			...
			logger.warn("Couldn't write settings out to disk in TRECCounting (.res.settings)", ioe);
		}
	}
